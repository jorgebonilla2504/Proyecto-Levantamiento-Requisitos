...
export default function UploadCourses() {
  ...
  function uploadCoursesJson() {
    ...
    fetch(Config.api_url + 'InsertarCursosDesdeJson', requestOptions).then(
      (response) => {
        if (!response.ok) {
  ...
  return (
    <>
      ...
      <div className="main-container">
        ...
        <p>
          Por favor suba el archivo conteniendo los cursos para desplegar en los
          formularios de levantamiento
        </p>
        <p>
          Tenga en cuenta que este archivo debe ser de extensión .xlsx para el
          manejo correcto de los datos.
        </p>
        <label htmlFor="period">
          Seleccione el periodo al que pertenecen los cursos
        </label>
        <br />
        <select
          className="input"
        ...
        />
        <br />
        <button className="button" onClick={uploadCoursesJson}>
          Cargar cursos
        </button>
      </div>
    </>
